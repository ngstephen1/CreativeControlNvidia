...
def main() -> None:
    ...
    # --- Render MV Button (calls FastAPI backend) ---
    st.markdown("#### Or render the full MV using your local GPU backend:")
    if st.button("🎬 Render Full Music Video (SVD backend)", key="btn_render_mv"):
        with st.spinner("Sending video plan to backend and rendering..."):
            try:
                resp = requests.post(
                    f"{RENDER_BACKEND_BASE}/render-mv",
                    json=video_plan,
                    timeout=3600,
                )
                if resp.status_code != 200:
                    ...
                    mv_result = resp.json()
                    mv_path = mv_result.get("mv_path") or str(FULL_MV_PATH)
                    if os.path.exists(mv_path):
                        st.success("Music video rendered!")
                        st.session_state["rendered_mv_path"] = mv_path
                    else:
                        st.warning("Backend said rendering was successful, but MV file not found yet.")
    ...
    mv_path = st.session_state.get("rendered_mv_path") or str(FULL_MV_PATH)
    if mv_path and os.path.exists(mv_path):
        st.video(str(mv_path))
        st.caption(
            "This final MV was rendered by the external image→video pipeline (e.g., Colab Stable Video Diffusion, or FastAPI SVD backend)."
        )
    else:
        st.info(
            "No final MV found yet. Run the Colab / GPU renderer or use the Render MV button above to write 'generated/final_music_video.mp4', then refresh this page."
        )
